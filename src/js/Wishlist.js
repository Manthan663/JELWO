...
const container = document.getElementById("wishlist-product");

let wishlist = JSON.parse(localStorage.getItem("wishlist")) || [];

function renderwishlist() {
  if (wishlist.length === 0) {
    container.innerHTML = `<p class="text-center col-span-full text-gray-500 text-xl">No Items in wishlist</p>`;
  ...
  container.innerHTML = wishlist
    .map((item) => {
      return `<div class="bg-gray-50 p-10 relative group hover:shadow-lg transition">
    ...
    <img src="${item.img}" class="w-full object-cover transition duration-500 group-hover:opacity-30">
      ...
      <div data-id="${item.id}" class="wishlist-btn w-10 h-10 border rounded-full flex items-center justify-center text-white   cursor-pointer">
  ...
  <button class="absolute top-0 left-0 remove-btn bg-red-500 text-white px-3 py-1 mt-2" data-id = ${item.id}>Remove</button>
  </div>
